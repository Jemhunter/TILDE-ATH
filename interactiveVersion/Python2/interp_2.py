...
global file#
file = open("data\data.txt", "w").close()#
file2 = open("data\data2.txt", "w").close()#
...
def killObj(name):##
    keys = list(ATHVars.keys())
    if name in keys:
        obj = ATHVars.get(name)
        obj.DIE()
        fileStore()
...
def fileStore():##
    file2 = open("data\data2.txt", "w")
    keys = list(ATHVars.keys())
    alive = []
    for i in keys:
        if ATHVars.get(i).living == True:
            alive.append(i)
    file2.write(str(alive))
    file2.close()
...
os.system("taskkill /F /IM py.exe")#needs to be corrected for all OS  
os.system("taskkill /F /IM python.exe")#needs to be corrected for all OS   
